...
export default function VendorSettlementDetailPage() {
  ...
  useEffect(() => {
    ...
    vendorService
      ...
      .then((res) => {
        if (!isMounted) return;
        const data = res?.data?.data || res?.data?.settlement || res?.data;
        setSettlement(data);
      })
      .catch((err) => {
        if (!isMounted) return;
    ...
    return (
      <div className="p-12 text-center bg-white rounded-3xl border border-slate-200 max-w-2xl mx-auto my-12">
        ...
        <p className="text-xs text-slate-500 mt-1 mb-4">
          This statement does not exist or does not belong to your vendor account.
        </p>
        <Link
          href="/vendor/finance"
    ...
    <div className="space-y-6 max-w-4xl mx-auto p-4 sm:p-6 pb-20">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs space-y-4">
        <h3 className="text-sm font-bold text-slate-900 border-b border-slate-100 pb-2 flex items-center gap-2">
          ...
          <span>Settlement Accounting Breakdown</span>
        </h3>
        ...
        {settlement.paidAt && (
          <div className="p-3 bg-emerald-50 rounded-2xl border border-emerald-200 text-xs text-emerald-800 flex items-center gap-2">
            ...
            <span>
              Disbursed to merchant bank account on <strong>{new Date(settlement.paidAt).toLocaleString("en-IN")}</strong>.
            </span>
          </div>
        )}
      </div>
      ...
      {orders.length > 0 && (
        <div className="bg-white rounded-3xl border border-slate-200/80 shadow-xs overflow-hidden">
          <div className="p-6 border-b border-slate-200/80">
            <h3 className="text-sm font-bold text-slate-900">Included Marketplace Orders ({orders.length})</h3>
            <p className="text-xs text-slate-500">Delivered orders that fulfilled the return eligibility policy for this cycle</p>
          </div>
                ...
                {orders.map((order, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/60 transition-colors">
                    <td className="px-6 py-4">
                      <Link
                        href={`/vendor/orders/${order.secureId || order._id}`}
