...
export function MobileStickyBar({ product }) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-slate-200 bg-white/95 backdrop-blur-md px-4 py-3 shadow-lg md:hidden">
      <div className="flex items-center justify-between gap-3">
        <div className="flex flex-col min-w-0">
          <span className="text-xs text-slate-500 line-clamp-1 font-medium">
            {product.name}
          </span>
          <span className="text-base font-black text-[#007A55]">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Button
            ...
            size="sm"
            className="rounded-full bg-[#007A55] hover:bg-[#006346] text-white font-bold text-xs px-3.5 py-2 shadow-xs cursor-pointer disabled:opacity-50"
          >
            {isAdding ? (
